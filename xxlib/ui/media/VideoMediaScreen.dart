...
class _VideoMediaState extends State<VideoMedia> {
  ...
  void getuserSavedData() async {
    ...
    print(userItems);
    topictest = prefs.getString("SavedTopics") == null
        ? ""
  ...
  Future<void> _fetchPage(int pageKey) async {
    try {
      ...
      final newItems = await MediaService()
          .getAllMedia(topicID: topictest, catID: 25, page: pageKey);
              ...
              Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                  builder: (context) => MediaDetails(
                      mediaD: item, userID: userID, Type: item.categoryName!),
                ),
              );
            },
            child: Padding(
                      ...
                      Padding(
                        ...
                        child: ClipRRect(
                          ...
                          child: VisibilityDetector(
                            // Must provide key
                            key: ValueKey<String>(
                                'give any string value to represent key'),
                            ...
                            onPressed: () async {
                              if (intbookmarks.contains(item.id) == true) {
                                var msg = await MediaService().unBookmarkMedia(
                                    userID: userID,
                                    parentID: int.parse(item.toString()));
                                //  print(msg);
                                setState(() {
                                  intbookmarks.remove(item.id);
                              ...
                              } else if (intbookmarks.contains(item.id) ==
                                  false) {
                                var msg = await MediaService().bookmarkMedia(
                                    userID: userID,
                                    parentID: int.parse(item.id.toString()));
                                //  print(msg);
                                setState(() {
